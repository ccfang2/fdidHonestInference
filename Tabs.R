...
Home <- tabPanel(strong("Home"), icon = icon("house"),
                 ...
                 p("This application is intended to facilitate the honest causal inference approach discussed in ", a("Fang and Liebl (2026).", href="https://arxiv.org/abs/2512.06804"), "That paper presents a novel functional perspective on Difference-in-Differences (DiD) that allows for honest inference using event study plots under violations of parallel trends and/or no-anticipation assumptions. 
                   Specifically, we compute an infimum-based simultaneous confidence band in the pre-treatment period by parametric bootstrap, and a supremum-based simultaneous confidence band in the post-treatment period by the algorithm of Kac-Rice formula proposed in ", a("Liebl and Reimherr (2023).", href="https://academic.oup.com/jrsssb/article/85/3/842/7133768"),  "Additionally, by contrast to classical reference line in traditional event study plots, we derive an honest reference band, accounting for potential biases from the violation of parallel trends or no-anticipation assumption, when making inference."),
                 p("By doing so, we turn traditional event study plots into rigorous honest causal inference tools through equivalence and relevance testing: Honest reference band can be validated via equivalence testing in the pre-anticipation period, and honest causal effects can be tested through relevance testing using the honest reference band in the post-treatment period. Below, you may find a video of short presentation for an early version of that paper."),
